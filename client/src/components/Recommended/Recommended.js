...
const Recommended = () => {
  ...
  const [loadingText, setLoadingText] = useState(
    "...get the 🍿 ready, I'm getting your movies..."
  );
  ...
  return preparedMovies === "processing" ? (
    <RecommendedContainer>
      <BeatLoader size={100} color={"#F65F2D"} />
    ...
    <RecommendedContainer>
      {loadingEffect ? (
        <GetBtn disabled onClick={getRecommendations}>
          <BeatLoader />
        </GetBtn>
      ) : (
        <GetBtn
          style={
            recommendButton
              ? { opacity: "1" }
              : { opacity: ".5", boxShadow: "none" }
          }
          onClick={getRecommendations}
        >
          {recommendButton
            ? "Generate Recommendations"
            : "Rate More Movies To Enable"}
        </GetBtn>
      )}
      ...
      {loadingEffect ? (
        ...
        <div>
          <FullViewContainer>
            {recommendedAPI.length > 0 && recommendedAPI[movieCounter] && (
              <RenderRecommendations
                ...
                releaseDate={
                  recommendedAPI[movieCounter].release_date &&
                  recommendedAPI[movieCounter].release_date.slice(0, 4)
                }
              />
            )}
          </FullViewContainer>
