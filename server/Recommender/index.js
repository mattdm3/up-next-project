...
let moviesMetaDataPromise = new Promise((resolve) =>
  fs
    .createReadStream('./recommender/data/movies_metadata.csv')
...
function init([moviesMetaData, moviesKeywords, ratings]) {
  ...
  console.log('(2) Prediction \n');
  console.log(sliceAndDice(cfItemBasedRecommendation, MOVIES_BY_ID, 50, true));
...
function sliceAndDice(recommendations, MOVIES_BY_ID, count, onlyTitle) {
  ...
  recommendations = onlyTitle
    ? recommendations.map(mr => ({ title: MOVIES_BY_ID[mr.movieId].title, score: mr.score, id: MOVIES_BY_ID[mr.id] }))
    : recommendations.map(mr => ({ movie: MOVIES_BY_ID[mr.movieId], score: mr.score, id: MOVIES_BY_ID[mr.id] }));
